...
const AI_AGENT_DEFAULT_BODY_HTML = `
    <div id="ai-agent-selection-bar" class="selection-bar is-hidden">
        <div class="selection-bar-copy">
            ...
            <span id="ai-agent-selection-detail" class="selection-bar-detail">Select records to prepare bulk actions.</span>
        </div>
    ...
    results.forEach((row, index) => {
        ...
        keys.forEach(k => {
            ...
            if ((k === 'image_url' || k === 'file_path') && typeof val === 'string' && val.startsWith('/static/')) {
                html += `<td><img src="${val}" alt="Template image" style="width: 56px; height: 56px; object-fit: cover; border-radius: 10px; border: 1px solid #d7deeb;"></td>`;
