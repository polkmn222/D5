...
from web.backend.app.services.message_template_service import MessageTemplateService
...
async def template_upload(template_id: str, file: UploadFile = File(...), db: Session = Depends(get_db)):
    try:
        upload_dir = Path(__file__).resolve().parents[4] / "app" / "static" / "uploads"
        ...
        return {"status": "success", "image_url": image_url, "name": filename}
    except Exception as e:
