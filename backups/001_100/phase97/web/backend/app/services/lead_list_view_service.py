...
class LeadListViewService:
    ...
    @classmethod
    def ensure_builtin_views(cls, db: Session, default_columns: list[str]) -> None:
        changed = False
        default_columns_json = json.dumps(default_columns)

        for builtin in cls.BUILTIN_VIEWS:
            record = (
                db.query(LeadListView)
                .filter(
                    LeadListView.id == builtin["id"],
                    LeadListView.owner_key == cls.OWNER_KEY,
                    LeadListView.object_type == cls.OBJECT_TYPE,
                )
                .first()
            )
    ...
    @classmethod
    def list_views(cls, db: Session, default_columns: list[str]) -> list[dict]:
        cls.ensure_builtin_views(db, default_columns)
        records = (
            db.query(LeadListView)
            .filter(
                LeadListView.owner_key == cls.OWNER_KEY,
    ...
    @classmethod
    def get_pinned_view_id(cls, db: Session, default_columns: list[str]) -> str | None:
        cls.ensure_builtin_views(db, default_columns)
        record = (
            db.query(LeadListView)
            .filter(
    ...
    @classmethod
    def create_view(cls, db: Session, payload: dict, default_columns: list[str]) -> dict:
        cls.ensure_builtin_views(db, default_columns)
    ...
    @classmethod
    def update_view(cls, db: Session, view_id: str, payload: dict, default_columns: list[str]) -> dict:
        cls.ensure_builtin_views(db, default_columns)
        record = (
            db.query(LeadListView)
            .filter(
    ...
    @classmethod
    def delete_view(cls, db: Session, view_id: str) -> None:
        record = (
            db.query(LeadListView)
            .filter(
                LeadListView.id == view_id,
    ...
    @classmethod
    def set_pinned_view(cls, db: Session, view_id: str, default_columns: list[str], pinned: bool = True) -> str | None:
        cls.ensure_builtin_views(db, default_columns)
        records = (
            db.query(LeadListView)
            .filter(
