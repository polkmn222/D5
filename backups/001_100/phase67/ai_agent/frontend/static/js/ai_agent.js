...
const AI_AGENT_DEFAULT_BODY_HTML = `
    <div id="ai-agent-selection-bar" class="selection-bar is-hidden">
        <div class="selection-bar-copy">
            ...
            <span id="ai-agent-selection-detail" class="selection-bar-detail">Keep chatting to manage these records, or choose Send Message only when you're ready.</span>
        </div>
...
function summarizeSelectionIds(ids) {
    if (!ids || !ids.length) return "Keep chatting to manage these records, or choose Send Message only when you're ready.";
...
function updateSelectionBar() {
    ...
    if (!selection) {
        ...
        detail.textContent = "Keep chatting to manage these records, or choose Send Message only when you're ready.";
        return;
    }
...
function openAgentImagePreview(url, title = 'Template Image') {
    ...
    if (!modal || !image || !heading || !url) return;
    image.src = url;
...
function renderAgentResultsMarkup(results, objectType, pagination = null, originalQuery = null) {
    ...
    results.forEach((row, index) => {
        ...
        if (hasTemplatePreview) {
            ...
            html += `<td style="display:flex;gap:8px;flex-wrap:wrap;">${previewUrl ? `<button class="control-btn" onclick="event.stopPropagation(); openAgentImagePreview('${escapeAgentQuery(previewUrl)}', 'Template Preview')">Preview</button>` : '<span style="color:#8a94a6;font-size:0.78rem;">No image</span>'}<button class="control-btn" onclick="event.stopPropagation(); startTemplateSendFromAgent('${rowId}')">Use In Send Message</button></td>`;
        }
        html += '</tr>';
    });
