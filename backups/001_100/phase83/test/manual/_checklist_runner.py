...
def write_rerun_report(report_name: str, title: str, results: list[dict[str, str]]) -> Path | None:
    failed = [result for result in results if result["status"] == "fail"]
    if not failed:
        return None

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = EVIDENCE_DIR / f"{report_name}_rerun_{timestamp}.md"
    lines = [f"# {title} Rerun Checklist", "", "Generated from failed manual checklist items.", "", "## Failed Items", ""]
    for result in failed:
        lines.extend(
            [
                f"### [{result['area']}] {result['label']}",
                ...
                f"- steps: {result['steps']}",
                f"- expected: {result['expected']}",
                f"- previous notes: {result['notes'] or '(none)'}",
                "",
            ]
        )
    path.write_text("\n".join(lines))
...
def run_checklist(title: str, intro: str, items: list[ChecklistItem], default_report_name: str) -> None:
    ...
    rerun_path = write_rerun_report(args.report_name, title, results)
    print(f"Saved report to {report_path}")
    if rerun_path is not None:
        print(f"Saved failed-item rerun checklist to {rerun_path}")
