...
function renderAgentResultsMarkup(results, objectType, pagination = null, originalQuery = null) {
    ...
    results.forEach((row, index) => {
        ...
        if (hasTemplatePreview) {
            ...
            html += `<td>${previewUrl ? `<button class="control-btn" onclick="event.stopPropagation(); openAgentImagePreview('${escapeAgentQuery(previewUrl)}', 'Template Preview')">Preview</button>` : '<span style="color:#8a94a6;font-size:0.78rem;">No image</span>'}</td>`;
        }
        html += '</tr>';
    });
