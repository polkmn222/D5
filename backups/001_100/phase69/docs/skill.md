...
This document describes the practical implementation guidance for the three active application surfaces: `backend`, `frontend`, and `ai_agent`.

## Backend

### Scope

- The main FastAPI application lives in `backend/app/`.
- Request composition begins in `backend/app/main.py` and `backend/app/api/api_router.py`.
- Business logic lives in `backend/app/services/`.
- Shared enums, template helpers, toggles, and utility code live under `backend/app/core/` and `backend/app/utils/`.
...
- Messaging flows: templates, sends, attachments, SureM integration, and broker debugging.
...
- Shared CRM templates live in `frontend/templates/`.
- Shared CSS and JavaScript live in `frontend/static/`.
- Template loading is configured centrally through `backend/app/core/templates.py`.
