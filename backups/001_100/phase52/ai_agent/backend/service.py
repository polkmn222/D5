...
class AiAgentService:
    ...
    @classmethod
    def _resolve_recent_reference(cls, user_query: str, conversation_id: Optional[str]) -> Optional[Dict[str, Any]]:
        if not conversation_id:
            return None

        q_low = user_query.lower()
        recent_markers = ["just created", "recently created", "방금 생성", "방금 만든"]
        if not any(marker in q_low or marker in user_query for marker in recent_markers):
            return None
    ...
    @classmethod
    async def process_query(cls, db: Session, user_query: str, conversation_id: Optional[str] = None) -> Dict[str, Any]:
        contextual_response = cls._resolve_recent_reference(user_query, conversation_id)
        if contextual_response:
            return await cls._execute_intent(db, contextual_response, user_query, conversation_id=conversation_id)
        ...
        if rule_based:
            return await cls._execute_intent(db, rule_based, user_query, conversation_id=conversation_id)
        ...
        try:
            return await cls._execute_intent(db, agent_output, user_query, conversation_id=conversation_id)
        except Exception as e:
        ...
        if intent == "MODIFY_UI":
            ...
            if "ai recommend" in q_low or "추천" in q_low:
                if "hot" in q_low or "따끈" in q_low:
                    ...
                    agent_output["text"] = "How should I filter the **AI Recommended Deals** on your home screen? \n\nOptions: [Hot Deals], [High Value], [Closing Soon], or [Default]."
            ...
            if any(word in q_low for word in ["compact", "축소", "작게"]):
                ...
                agent_output["intent"] = "CHAT"
                agent_output["text"] = "How would you like to change the table format? You can choose [Compact], [Modern], or [Default] style. Or describe a new logic for the [AI Recommend] table!"
