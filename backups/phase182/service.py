...
class AiAgentService:
    ...
    def _default_query_parts(obj: str) -> Optional[Dict[str, str]]:
        mapping = {
            ...
            "contact": {
                "select": "id, TRIM(CONCAT_WS(' ', first_name, last_name)) AS display_name, phone, email, tier, created_at",
                "from": "contacts",
                "where": "deleted_at IS NULL",
                "order_by": "created_at DESC",
            },
            "opportunity": {
                "select": "o.id, o.name, TRIM(CONCAT_WS(' ', c.first_name, c.last_name)) AS contact_display_name, c.phone AS contact_phone, o.stage, o.amount, COALESCE(m.name, o.model) AS model_name",
    ...
    def _resolve_delete_confirmation(
        ...
        # Phase 177: Try to extract explicit record ID from query if not provided in selection
    ...
    async def process_query(
        ...
        DELETE FLOW:
        - If the user provides a record ID (e.g., "Delete lead 123") or confirms deletion (e.g., "Yes"), use intent "DELETE" immediately.
        - The UI already handles "Are you sure?" confirmations. Do NOT ask for confirmation again. Proceed directly to the deletion.
