    ...
    if (role === 'user') {
        ...
        if ((trimmed === 'yes') && window._agentPendingDelete) {
            ...
            if (pending.ids && pending.ids.length > 1) {
                sendAiMessage(`[FORCE_DELETE] Delete selected ${pending.objectType} records`);
            } else {
                sendAiMessage(`[FORCE_DELETE] Delete ${pending.objectType} ${pending.ids ? pending.ids[0] : pending.recordId}`);
            }
            return;
        }
