...
def test_message_list_template_contains_custom_list_view_controls():
    ...
    assert ">Edit<" in html
    assert ">Del<" in html
    assert 'style="background: #fff;"' in html
...
def test_message_detail_template_uses_related_cards_for_connected_records():
    html = MESSAGE_DETAIL_TEMPLATE.read_text()
    assert "sf-related-card" in html
    assert "Connected records for this message" in html
    assert "View All" in html


def test_message_detail_template_is_read_only():
    html = MESSAGE_DETAIL_TEMPLATE.read_text()
    assert "toggleInlineEdit(" not in html
    assert 'id="edit-' not in html
    assert "<div class=\"sf-field-label\">Content</div>" in html
...
def test_message_batch_save_route_supports_message_object_type():
    Base.metadata.create_all(bind=engine)
    with patch("web.message.backend.services.message_service.MessageService.get_message", return_value=SimpleNamespace(status="Sent", content="Hello", template="tpl-1")), patch(
        "web.message.backend.services.message_service.MessageService.update_message", return_value=SimpleNamespace(status="Delivered")
    ):
        with TestClient(app) as client:
            response = client.post("/messages/msg-1/batch-save", json={"Status": "Delivered", "Template Hidden Ref": "tpl-2"})

    assert response.status_code == 200
    assert response.json() == {"status": "success"}


def test_message_crud_routes_support_modal_edit_and_delete():
    Base.metadata.create_all(bind=engine)
    with patch("web.message.backend.routers.message_router.MessageService.create_message", return_value=SimpleNamespace(id="msg-new")), patch(
        "web.message.backend.routers.message_router.MessageService.update_message", return_value=SimpleNamespace(id="msg-1")
    ), patch(
        "web.message.backend.routers.message_router.MessageService.delete_message", return_value=True
    ):
        with TestClient(app) as client:
            create_response = client.post("/messages/", data={"contact": "0031", "template": "tpl-1", "direction": "Outbound", "content": "Hello", "status": "Sent"}, follow_redirects=False)
            update_response = client.post("/messages/msg-1", data={"contact": "", "template": "tpl-2", "direction": "Inbound", "content": "Updated", "status": "Delivered"}, follow_redirects=False)
            delete_response = client.post("/messages/msg-1/delete", follow_redirects=False)

    assert create_response.status_code == 303
    assert create_response.headers["location"] == "/messages/msg-new?success=Record+created+successfully"
    assert update_response.status_code == 303
    assert update_response.headers["location"] == "/messages/msg-1?success=Record+updated+successfully"
    assert delete_response.status_code == 303
    assert delete_response.headers["location"] == "/messages?success=Record+deleted+successfully"
