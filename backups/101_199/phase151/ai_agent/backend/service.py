...
class AiAgentService:
    ...
    LEAD_UPDATE_FIELD_ALIASES = {
        ...
        "last_name": ["last name", "lastname", "surname", "성"],
        "status": ["status", "상태"],
        "email": ["email", "이메일", "메일"],
    ...
    def _extract_lead_update_fields_from_text(cls, user_query: str) -> Dict[str, Any]:
        ...
        extra_patterns = {
            "first_name": [r"first name\s*(?:is|to|=|:)?\s*([A-Za-z가-힣-]+)", r"이름(?:은|는|을|를)?\s*([A-Za-z가-힣-]+)"],
            "last_name": [r"last name\s*(?:is|to|=|:)?\s*([A-Za-z가-힣-]+)", r"성(?:은|는|을|를)?\s*([A-Za-z가-힣-]+)"],
            "gender": [r"gender\s*(?:is|to|=|:)?\s*([A-Za-z가-힣-]+)", r"성별(?:은|는|을|를)?\s*([A-Za-z가-힣-]+)"],
            "description": [r"description\s*(?:is|to|=|:)?\s*(.+)", r"(?:note|메모|설명)(?:는|은|을|를)?\s*(.+)"],
        }
    ...
    async def _execute_intent(
        ...
        if intent == "MANAGE":
            ...
            if obj in ["lead", "leads"]:
                ...
                if lead:
                    ...
                    lead_name = " ".join(part for part in [getattr(lead, "first_name", None), getattr(lead, "last_name", None)] if part).strip() or record_id
                    record_details = f"Lead: {lead_name} ({lead.status})"
                    agent_output["chat_card"] = cls._build_lead_chat_card(db, lead, mode=mode)
                    agent_output["text"] = (
                        f"I pasted the current lead snapshot for **{lead_name}** below. You can keep working in chat."
                        if mode != "edit" else
                        f"I pasted the editable lead card for **{lead_name}** below. Tell me what to change in chat."
                    )
        ...
        if intent == "CREATE":
            if obj == "lead":
                ...
                lead_name = " ".join(part for part in [getattr(res, "first_name", None), getattr(res, "last_name", None)] if part).strip() or "the new lead"
                agent_output["text"] = f"Success! Created Lead {lead_name}. I pasted the new lead card below so you can keep going in chat."
                agent_output["chat_card"] = cls._build_lead_chat_card(db, res, mode="edit")
                ConversationContextStore.clear_pending_create(conversation_id)
        ...
        if intent == "UPDATE" and record_id:
            if obj == "lead":
                ...
                if res:
                    ...
                    agent_output["text"] = f"Success! Updated Lead {record_id}. I pasted the refreshed lead card below."
                    if refreshed:
                        agent_output["chat_card"] = cls._build_lead_chat_card(db, refreshed, mode="edit")
