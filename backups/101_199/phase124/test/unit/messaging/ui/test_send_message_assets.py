...
def test_send_message_template_contains_duplicate_review_and_template_controls():
    ...
    assert "ai-recommend-mode-modal" in html
    assert "Choose AI Recommend Mode" in html
    assert "Show Recommendations" in html
...
def test_send_message_template_contains_phone_dedupe_logic_and_ai_selection_import():
    ...
    assert "modal-tpl-content-meta" in html
    assert "modal-tpl-subject-meta" in html
    assert "updateTemplateModalCounters" in html
    assert "subjectInput.disabled = type === 'SMS'" in html
    assert "modal-tpl-image-loading" in html
    assert "modal-tpl-image-help" in html
    assert "modal-tpl-image-error" in html
    assert "openAiRecommendModeModal" in html
    assert "selectAiRecommendMode" in html
    assert "applyAiRecommendMode" in html
    assert "Pick the type of opportunities you want to show in Send Message." in html
