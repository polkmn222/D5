...
3. Record detail pages preserve functional `Details`, `Related`, and `Activity` tabs where that layout is already part of the object template family.
...
- [ ] **Audit Fields**: `created_at` and `updated_at` continue to populate automatically and remain non-editable from normal UI flows.
