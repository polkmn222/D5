...
def test_delete_request_requires_confirmation_for_all_objects(
    ...
    assert "[yes]" in data["text"]
    assert "[cancel]" in data["text"]
...
def test_multi_delete_confirmation_executes_selected_records(client, monkeypatch):
    conversation_id = "conv-delete-multi"
    ConversationContextStore.clear(conversation_id)

    async def fail_llm(user_query: str, system_prompt: str):
        raise AssertionError("LLM should not be called for multi delete confirmation")
    ...
    monkeypatch.setattr("web.backend.app.services.contact_service.ContactService.delete_contact", fake_delete)

    first = client.post(
        "/api/chat",
        json={
            "query": "Delete selected contact records",
            "conversation_id": conversation_id,
            "selection": {"object_type": "contact", "ids": ["contact-1", "contact-2"]},
        },
    )
    assert "Delete confirmation needed" in first.json()["text"]

    confirm = client.post("/api/chat", json={"query": "yes", "conversation_id": conversation_id})
    data = confirm.json()

    assert calls == ["contact-1", "contact-2"]
    assert data["intent"] == "DELETE"
    assert "Deleted 2 of 2 Contacts" in data["text"]
