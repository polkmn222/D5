...
def test_opportunity_filtered_route_uses_related_mode_without_full_list_controls():
    Base.metadata.create_all(bind=engine)
    with patch("web.backend.app.api.routers.opportunity_router.OpportunityService.get_opportunities", return_value=[]):
        with TestClient(app) as client:
            response = client.get("/opportunities?contact=contact-1")

    assert response.status_code == 200
    assert "Related List" in response.text
    assert "Back" in response.text
    assert "New" in response.text
    assert "Setup" not in response.text
    assert "Pinned" not in response.text
    assert "/opportunities/new-modal?contact=contact-1" in response.text
...
def test_opportunity_detail_related_lists_include_contact_product_asset_model_and_brand():
    ...
    assert "related=1" in response.text
    assert "back_to=/opportunities/opp-1" in response.text
    ...
    assert "sf-related-icon--contact" in response.text
    assert "₩ 550,000" in response.text or "₩ 300,000" in response.text
