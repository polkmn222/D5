...
async def list_models(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        total_items = db.query(Model).filter(Model.deleted_at == None).count()
        total_pages = max((total_items + MODEL_PAGE_SIZE - 1) // MODEL_PAGE_SIZE, 1)
        if page > total_pages:
            page = total_pages
        models_data = (
            db.query(Model)
