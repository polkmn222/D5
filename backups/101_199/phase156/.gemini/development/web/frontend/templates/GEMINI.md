...
* **Router Structure**: Within the `router/` folder, each class must correspond to exactly one `@router` instance.
* **Service Structure**: Within the `service/` folder, use a class-based structure where functions are defined as `@staticmethod`.
