...
async def test_manage_lead_intent_returns_correct_card_and_record_id(mock_db):
    ...
    with patch("ai_agent.backend.service.LeadService.get_lead", return_value=mock_lead):
        with patch("ai_agent.backend.service.AiAgentService._call_multi_llm_ensemble", return_value={"intent": "MANAGE", "object_type": "lead", "record_id": "123"}):
            with patch("ai_agent.backend.service.AiAgentService._build_lead_chat_card", return_value={"type": "lead_paste"}):
...
async def test_create_lead_returns_chat_card(mock_db):
    ...
    with patch("ai_agent.backend.service.IntentPreClassifier.detect", return_value=None):
        with patch("ai_agent.backend.service.AiAgentService._call_multi_llm_ensemble", return_value={"intent": "CREATE", "object_type": "lead", "data": {"last_name": "Lead", "status": "New"}}):
            with patch("ai_agent.backend.service.LeadService.create_lead", return_value=mock_lead):
                with patch("ai_agent.backend.service.AiAgentService._build_lead_chat_card", return_value={"type": "lead_paste"}):
                    response = await AiAgentService.process_query(
                        db=mock_db,
