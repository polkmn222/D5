...
async function resetAiAgent() {
    ...
    applyAiAgentLanguageUi();
    updateSelectionBar();
    if (msgDiv) scrollToElement(msgDiv);
}
...
async function sendAiMessage(queryOverride = null, pageOverride = 1) {
    ...
    try {
        ...
        // Sync local tables after deletion
        if (data.intent === 'DELETE' && data.text && data.text.toLowerCase().includes('success')) {
            const deletedIds = data.record_id ? [data.record_id] : (data.selection?.ids || []);
            if (deletedIds.length > 0) {
                Object.keys(localAgentResultTables).forEach(tableKey => {
                    const tableState = localAgentResultTables[tableKey];
                    const originalCount = tableState.results.length;
                    tableState.results = tableState.results.filter(row => !deletedIds.includes(String(row.id)));
                    if (tableState.results.length !== originalCount) {
                        document.querySelectorAll(`.results-container[data-table-key="${tableKey}"]`).forEach(container => {
                            const newMarkup = renderLocalResultsTable(tableKey, 1);
                            if (newMarkup) container.outerHTML = newMarkup;
                        });
                    }
                });
            }
...
function renderAgentChatCard(card) {
    ...
    if (card.actions && card.actions.length > 0) {
        actionMarkup = `
            <div class="agent-paste-actions" style="padding: 12px; border-top: 1px solid #d7deeb; display: flex; gap: 8px;">
                ${card.actions.map(a => `
                    <button class="control-btn ${a.tone === 'danger' ? 'control-btn-danger' : 'control-btn-primary'}" 
                        onclick="sendQuickMessage('${escapeAgentHtml(a.action)} this ${escapeAgentHtml(card.object_type)}')">
                        ${escapeAgentHtml(a.label)}
                    </button>
                `).join('')}
            </div>
        `;
    ...
    return `
        <div class="agent-paste-card" data-agent-card-type="${escapeAgentHtml(card.type)}">
            <div class="agent-paste-meta">${escapeAgentHtml(card.paste_label || 'Snapshot')}</div>
...
function filterLocalAgentTable(tableKey, query) {
    const tableState = localAgentResultTables[tableKey];
    if (!tableState) return;
...
function renderAgentResultsMarkup(results, objectType, pagination = null, originalQuery = null) {
    ...
    const tableKey = pagination?.table_key || '';
    
    let html = `
        <div class="results-container" data-object-type="${objectType || ''}" ${tableKey ? `data-table-key="${tableKey}"` : ''}>
            <div style="font-size: 0.75rem; color: #706e6b; margin-bottom: 0.5rem; font-weight: 600;">Select records to take action.</div>
            <div class="table-controls" style="flex-wrap: wrap; gap: 8px; align-items: center; display: flex;">
                ...
                <button class="control-btn" onclick="triggerSelectionOpen()" style="margin-left: 8px;">📖 Open</button>
                <button class="control-btn" onclick="triggerSelectionEdit()" style="margin-left: 4px;">✏️ Edit</button>
                <button class="control-btn" onclick="triggerSelectionDelete()" style="margin-left: 4px; color: var(--error);">🗑️ Delete</button>
                
                <div style="margin-left: auto; display: flex; align-items: center; gap: 8px;">
                    <input type="text" placeholder="Search in table..." class="agent-table-search" 
                        style="padding: 4px 8px; font-size: 0.75rem; border: 1px solid #dddbda; border-radius: 4px; width: 150px;"
                        oninput="filterLocalAgentTable('${tableKey}', this.value)">
                    <span style="font-size: 0.75rem; color: #706e6b;">Click headers to sort</span>
                </div>
            </div>
            <div style="margin-top: 4px; margin-bottom: 8px;">
                <span class="agent-selection-label" style="font-size: 0.75rem; color: #0176d3;">Selected: <strong>${selectedIds.length}</strong> items</span>
            </div>
...
function triggerSelectionOpen() {
    ...
    if (selection.ids.length > 1) {
        appendChatMessage('agent', 'Please select only one record to Open.');
        return;
    }
...
function triggerSelectionEdit() {
    ...
    if (selection.ids.length > 1) {
        appendChatMessage('agent', 'Please select only one record to Edit.');
        return;
    }
    const label = selection.labels?.[0] || selection.ids[0];
    if (shouldUseAgentChatPaste(selection.object_type)) {
...
function triggerSelectionDelete() {
    ...
    const count = selection.ids.length;
    const label = normalizeObjectLabel(selection.object_type, count);
    const names = (selection.labels || []).slice(0, 3).join(', ');
    const suffix = selection.ids.length > 3 ? '...' : '';
    
    appendChatMessage('agent', `Are you sure you want to delete ${count} ${label} (${names}${suffix})? [Yes] [Cancel]`);
}
    const count = selection.ids.length;
    const label = normalizeObjectLabel(selection.object_type, count);
    const names = (selection.labels || []).slice(0, 3).join(", ");
