...
- `test/unit/ui/shared/test_batch_edit_ui.py` still contains legacy path assumptions for `messages/detail_view.html` and `message_templates/detail_view.html` under `web/frontend/templates/`; update these to respect `web/message/frontend/templates/`.
- `test/unit/ui/shared/test_inline_edit_unity.py` should keep shared-edit expectations for editable pages only and carve out the documented read-only message detail exception.
- `test/unit/messaging/router/test_upload_validation.py` should be kept in sync with the active direct-call signatures for `template_upload(...)` and `clear_template_image(...)`.
