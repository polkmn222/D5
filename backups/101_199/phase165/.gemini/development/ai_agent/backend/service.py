...
class AiAgentService:
    ...
    @classmethod
    def _build_chat_card(cls, db: Session, obj_type: str, record: Any, mode: str = "view", language_preference: Optional[str] = None) -> Dict[str, Any]:
        from web.backend.app.services.product_service import ProductService
        is_korean = (language_preference or "").lower() == "kor"

        fields = []
        title = "Record"
        subtitle = obj_type.replace("_", " ").title()
        record_id = str(getattr(record, "id", ""))

        if obj_type in ["lead", "leads"]:
            brand = VehicleSpecService.get_vehicle_spec(db, record.brand) if getattr(record, "brand", None) else None
            model = ModelService.get_model(db, record.model) if getattr(record, "model", None) else None
            product = ProductService.get_product(db, record.product) if getattr(record, "product", None) else None
            title = cls._lead_name(record)
            subtitle = f"Lead · {cls._display_value(getattr(record, 'status', None))}"
            fields = [
                {"label": "First name", "value": cls._display_value(getattr(record, "first_name", None))},
                {"label": "Last name", "value": cls._display_value(getattr(record, "last_name", None))},
                {"label": "Status", "value": cls._display_value(getattr(record, "status", None))},
                {"label": "Email", "value": cls._display_value(getattr(record, "email", None))},
                {"label": "Phone", "value": cls._display_value(getattr(record, "phone", None))},
                {"label": "Gender", "value": cls._display_value(getattr(record, "gender", None))},
                ...
                {"label": "Description", "value": cls._display_value(getattr(record, "description", None))},
            ]
        elif obj_type in ["contact", "contacts"]:
            title = f"{getattr(record, 'first_name', '')} {getattr(record, 'last_name', '')}".strip() or "Unnamed Contact"
            subtitle = f"Contact · {cls._display_value(getattr(record, 'tier', None))}"
            fields = [
                {"label": "First name", "value": cls._display_value(getattr(record, "first_name", None))},
                {"label": "Last name", "value": cls._display_value(getattr(record, "last_name", None))},
                {"label": "Email", "value": cls._display_value(getattr(record, "email", None))},
                {"label": "Phone", "value": cls._display_value(getattr(record, "phone", None))},
                {"label": "Tier", "value": cls._display_value(getattr(record, "tier", None))},
                {"label": "Last Interaction", "value": cls._display_value(getattr(record, "last_interaction_at", None))},
            ]
        elif obj_type in ["opportunity", "opportunities", "opps"]:
            title = getattr(record, "name", "Unnamed Opportunity")
            subtitle = f"Opportunity · {cls._display_value(getattr(record, 'stage', None))}"
            amount_val = getattr(record, "amount", 0)
            formatted_amount = f"₩{amount_val:,}" if amount_val else "₩0"
            fields = [
                {"label": "Name", "value": title},
                {"label": "Stage", "value": cls._display_value(getattr(record, "stage", None))},
                {"label": "Amount", "value": formatted_amount},
                {"label": "Temperature", "value": cls._display_value(getattr(record, "temperature", None))},
                {"label": "Close Date", "value": cls._display_value(getattr(record, "close_date", None))},
                {"label": "Followed", "value": cls._display_value(getattr(record, "is_followed", None))},
            ]
        elif obj_type in ["product", "products"]:
            title = getattr(record, "name", "Unnamed Product")
            subtitle = "Product"
            price_val = getattr(record, "base_price", 0)
            formatted_price = f"₩{price_val:,}" if price_val else "₩0"
            fields = [
                {"label": "Name", "value": title},
                {"label": "Brand", "value": cls._display_value(getattr(record, "brand", None))},
                {"label": "Model", "value": cls._display_value(getattr(record, "model", None))},
                {"label": "Category", "value": cls._display_value(getattr(record, "category", None))},
                {"label": "Base Price", "value": formatted_price},
            ]
        elif obj_type in ["asset", "assets"]:
            title = getattr(record, "vin", "No VIN")
            subtitle = f"Asset · {cls._display_value(getattr(record, 'status', None))}"
            fields = [
                {"label": "VIN", "value": title},
                {"label": "Brand", "value": cls._display_value(getattr(record, "brand", None))},
                {"label": "Model", "value": cls._display_value(getattr(record, "model", None))},
                {"label": "Status", "value": cls._display_value(getattr(record, "status", None))},
                {"label": "Purchase Date", "value": cls._display_value(getattr(record, "purchase_date", None))},
            ]
        elif obj_type in ["brand", "brands"]:
            title = getattr(record, "name", "Unnamed Brand")
            subtitle = "Brand"
            fields = [
                {"label": "Name", "value": title},
                {"label": "Type", "value": cls._display_value(getattr(record, "record_type", None))},
                {"label": "Description", "value": cls._display_value(getattr(record, "description", None))},
            ]
        elif obj_type in ["model", "models"]:
            title = getattr(record, "name", "Unnamed Model")
            subtitle = "Model"
            fields = [
                {"label": "Name", "value": title},
                {"label": "Brand", "value": cls._display_value(getattr(record, "brand", None))},
        ...
        elif obj_type in ["message_template", "template"]:
            title = getattr(record, "name", "Unnamed Template")
            subtitle = "Message Template"
            fields = [
                {"label": "Name", "value": title},
                {"label": "Subject", "value": cls._display_value(getattr(record, "subject", None))},
                {"label": "Content", "value": cls._display_value(getattr(record, "content", None))},
                {"label": "Image URL", "value": cls._display_value(getattr(record, "image_url", None))},
            ]

        line_count = 2 + len(fields)
        if is_korean:
            hint = (
                f"변경할 필드를 입력해 주세요. 예: `status Qualified`, `phone 01012345678`."
                if mode == "edit" else
                f"채팅에서 수정을 이어가려면 `이 {subtitle.split(' · ')[0]} 수정해줘`라고 말하거나 메시지 전송을 요청하세요."
            )
        else:
            hint = (
                f"Reply with the fields to change, like `status Qualified`, `phone 01012345678`."
                if mode == "edit" else
                f"Reply with `edit this {obj_type}` to keep updating in chat, or ask to send a message."
            )
        ...
        return {
            ...
            "mode": mode,
            "paste_label": f"Snapshot ~{line_count} lines",
            "title": title,
            "subtitle": subtitle,
            "record_id": record_id,
            "fields": fields,
    ...
    def _default_query_parts(obj: str) -> Optional[Dict[str, str]]:
        mapping = {
            ...
            "opportunity": {
                "select": "id, name, stage, temperature, amount, status, is_followed, close_date, updated_at",
                "from": "opportunities",
                "where": "deleted_at IS NULL",
                "order_by": "created_at DESC",
            },
    ...
    def _resolve_delete_confirmation(
        ...
        if pending_delete:
            if any(token in q_low for token in ["yes", "confirm", "proceed", "delete it", "yes delete", "네", "예", "삭제해", "삭제해줘"]):
            ...
            if any(token in q_low for token in ["cancel", "stop", "no", "never mind", "아니", "취소", "그만", "안해"]):
                ConversationContextStore.clear_pending_delete(conversation_id)
                return {
                    "intent": "CHAT",
                    "text": "리드 삭제 요청이 취소되었습니다." if is_korean else "Delete request cancelled.",
                    "score": 1.0,
                }
        ...
        if selected_object and selected_ids:
            ...
            if is_korean:
                prompt = (
                    f"삭제 확인이 필요합니다: {count}개의 {label} 레코드({preview_names})를 영구적으로 삭제할까요? "
                    "[예]를 선택하면 삭제하고 [취소]를 선택하면 유지합니다."
                    if count > 1 else
                    f"삭제 확인이 필요합니다: {selected_labels[0] if selected_labels else f'이 {label} 레코드'}를 영구적으로 삭제할까요? "
                    "[예]를 선택하면 삭제하고 [취소]를 선택하면 유지합니다."
                )
            else:
                prompt = (
        ...
        if is_korean:
            prompt = f"삭제 확인이 필요합니다: 이 {label} 레코드({record_id})를 영구적으로 삭제할까요? [예]를 선택하면 삭제하고 [취소]를 선택하면 유지합니다."
        else:
            prompt = f"Delete confirmation needed: should I permanently delete this {label} record ({record_id})? Choose [yes] to continue or [cancel] to keep it."
        return {
            "intent": "CHAT",
            "object_type": object_type,
            "record_id": record_id,
    ...
    async def process_query(
        ...
        delete_resolution = cls._resolve_delete_confirmation(user_query, conversation_id, selection, language_preference=language_preference)
        if delete_resolution:
            return await cls._execute_intent(db, delete_resolution, user_query, conversation_id=conversation_id, page=page, per_page=per_page, language_preference=language_preference)

        explicit_manage_resolution = cls._resolve_explicit_manage_request(user_query)
        if explicit_manage_resolution:
            return await cls._execute_intent(db, explicit_manage_resolution, user_query, conversation_id=conversation_id, page=page, per_page=per_page, language_preference=language_preference)

        contextual_response = cls._resolve_contextual_record_reference(user_query, conversation_id)
        if contextual_response:
            return await cls._execute_intent(db, contextual_response, user_query, conversation_id=conversation_id, page=page, per_page=per_page, language_preference=language_preference)
        ...
        if rule_based:
            ...
            return await cls._execute_intent(db, rule_based, user_query, conversation_id=conversation_id, page=page, per_page=per_page, language_preference=language_preference)
        ...
        try:
            return await cls._execute_intent(db, agent_output, user_query, conversation_id=conversation_id, page=page, per_page=per_page, language_preference=language_preference)
        except Exception as e:
    ...
    async def _execute_intent(
            ...
            if obj in ["lead", "leads"]:
                ...
                if lead:
                    ...
                    if mode == "edit":
                        ...
                        return cls._build_lead_edit_form_response(record_id, lead_name, language_preference)
                    record_details = f"Lead: {lead_name}"
                    agent_output["chat_card"] = cls._build_chat_card(db, "lead", lead, mode=mode, language_preference=language_preference)
            elif obj in ["contact", "contacts"]:
                contact = ContactService.get_contact(db, record_id)
                if contact:
                    record_details = f"Contact: {getattr(contact, 'first_name', '')} {getattr(contact, 'last_name', '')}"
                    agent_output["chat_card"] = cls._build_chat_card(db, "contact", contact, mode=mode, language_preference=language_preference)
            elif obj in ["opportunity", "opportunities", "opps"]:
                opp = OpportunityService.get_opportunity(db, record_id)
                if opp:
                    record_details = f"Opportunity: {opp.name}"
                    agent_output["chat_card"] = cls._build_chat_card(db, "opportunity", opp, mode=mode, language_preference=language_preference)
            elif obj in ["brand", "brands"]:
                spec = VehicleSpecService.get_vehicle_spec(db, record_id)
                if spec:
                    record_details = f"Brand: {spec.name}"
                    agent_output["chat_card"] = cls._build_chat_card(db, "brand", spec, mode=mode, language_preference=language_preference)
            elif obj in ["model", "models"]:
                model = ModelService.get_model(db, record_id)
                if model:
                    record_details = f"Model: {model.name}"
                    agent_output["chat_card"] = cls._build_chat_card(db, "model", model, mode=mode, language_preference=language_preference)
            elif obj in ["product", "products"]:
                from web.backend.app.services.product_service import ProductService
                prod = ProductService.get_product(db, record_id)
                if prod:
                    record_details = f"Product: {prod.name}"
                    agent_output["chat_card"] = cls._build_chat_card(db, "product", prod, mode=mode, language_preference=language_preference)
            elif obj in ["asset", "assets"]:
                from web.backend.app.services.asset_service import AssetService
                asset = AssetService.get_asset(db, record_id)
                if asset:
                    record_details = f"Asset: {asset.vin}"
                    agent_output["chat_card"] = cls._build_chat_card(db, "asset", asset, mode=mode, language_preference=language_preference)
            elif obj in ["message_template", "template"]:
                template = MessageTemplateService.get_template(db, record_id)
                if template:
                    record_details = f"Template: {template.name}"
                    agent_output["chat_card"] = cls._build_chat_card(db, "message_template", template, mode=mode, language_preference=language_preference)
            
            if record_details and agent_output.get("chat_card"):
                agent_output["record_id"] = record_id
                agent_output["text"] = f"I pasted the current snapshot for **{record_details}** below. You can keep working in chat."
        ...
        if intent == "CREATE":
            if obj == "lead":
                ...
                agent_output["auto_open"] = True
                agent_output["text"] = f"Success! Created lead {lead_name}. Opening details..."
                agent_output["chat_card"] = cls._build_chat_card(db, "lead", res, mode="view", language_preference=language_preference)
                ConversationContextStore.clear_pending_create(conversation_id)
                ...
                agent_output["text"] = f"Success! Created Contact {name}. I pasted the current snapshot below."
                agent_output["chat_card"] = cls._build_chat_card(db, "contact", res, mode="view", language_preference=language_preference)
                ConversationContextStore.remember_created(conversation_id, obj, str(res.id))
                return agent_output
            elif obj == "opportunity":
                res = OpportunityService.create_opportunity(db, **data)
                agent_output["text"] = f"Success! Created Opportunity {res.name}. I pasted the current snapshot below."
                agent_output["chat_card"] = cls._build_chat_card(db, "opportunity", res, mode="view", language_preference=language_preference)
                ConversationContextStore.remember_created(conversation_id, obj, str(res.id))
                return agent_output
            elif obj == "brand":
                data["record_type"] = "Brand"
                res = VehicleSpecService.create_spec(db, **data)
                agent_output["text"] = f"Success! Created Brand {res.name}. I pasted the current snapshot below."
                agent_output["chat_card"] = cls._build_chat_card(db, "brand", res, mode="view", language_preference=language_preference)
                ConversationContextStore.remember_created(conversation_id, obj, str(res.id))
                return agent_output
            elif obj == "model":
                res = ModelService.create_model(db, **data)
                agent_output["text"] = f"Success! Created Model {res.name}. I pasted the current snapshot below."
                agent_output["chat_card"] = cls._build_chat_card(db, "model", res, mode="view", language_preference=language_preference)
                ConversationContextStore.remember_created(conversation_id, obj, str(res.id))
                return agent_output
            elif obj == "product":
                from web.backend.app.services.product_service import ProductService
                res = ProductService.create_product(db, **data)
                agent_output["text"] = f"Success! Created Product {res.name}. I pasted the current snapshot below."
                agent_output["chat_card"] = cls._build_chat_card(db, "product", res, mode="view", language_preference=language_preference)
                ConversationContextStore.remember_created(conversation_id, obj, str(res.id))
                return agent_output
            elif obj == "asset":
                from web.backend.app.services.asset_service import AssetService
                res = AssetService.create_asset(db, **data)
                agent_output["text"] = f"Success! Registered Asset with VIN {res.vin}. I pasted the current snapshot below."
                agent_output["chat_card"] = cls._build_chat_card(db, "asset", res, mode="view", language_preference=language_preference)
                ConversationContextStore.remember_created(conversation_id, obj, str(res.id))
                return agent_output
            elif obj in ["message_template", "template"]:
                name = data.pop("name", "New Template")
                res = MessageTemplateService.create_template(db, name=name, **data)
                agent_output["text"] = f"Success! Created Template: {res.name}. I pasted the current snapshot below."
                agent_output["chat_card"] = cls._build_chat_card(db, "message_template", res, mode="view", language_preference=language_preference)
        ...
        if intent == "UPDATE" and record_id:
            if obj == "lead":
                ...
                if res:
                    ...
                    agent_output["record_id"] = str(record_id)
                    agent_output["auto_open"] = True
                    agent_output["text"] = f"Success! Updated Lead {record_id}. Refreshing details..."
                    if refreshed:
                        agent_output["chat_card"] = cls._build_chat_card(db, "lead", refreshed, mode="edit", language_preference=language_preference)
                ...
                if res:
                    ...
                    agent_output["text"] = f"Success! Updated Contact {record_id}. I pasted the refreshed card below."
                    if refreshed:
                        agent_output["chat_card"] = cls._build_chat_card(db, "contact", refreshed, mode="edit", language_preference=language_preference)
                else:
                ...
                if res:
                    ...
                    agent_output["text"] = f"Success! Updated Opportunity {record_id}. I pasted the refreshed card below."
                    if refreshed:
                        agent_output["chat_card"] = cls._build_chat_card(db, "opportunity", refreshed, mode="edit", language_preference=language_preference)
                else:
                ...
                if res:
                    ...
                    agent_output["text"] = f"Success! Updated Brand {record_id}. I pasted the refreshed card below."
                    if refreshed:
                        agent_output["chat_card"] = cls._build_chat_card(db, "brand", refreshed, mode="edit", language_preference=language_preference)
                else:
                ...
                if res:
                    ...
                    agent_output["text"] = f"Success! Updated Model {record_id}. I pasted the refreshed card below."
                    if refreshed:
                        agent_output["chat_card"] = cls._build_chat_card(db, "model", refreshed, mode="edit", language_preference=language_preference)
                else:
                ...
                if res:
                    ...
                    agent_output["text"] = f"Success! Updated Product {record_id}. I pasted the refreshed card below."
                    if refreshed:
                        agent_output["chat_card"] = cls._build_chat_card(db, "product", refreshed, mode="edit", language_preference=language_preference)
                else:
                ...
                if res:
                    ...
                    agent_output["text"] = f"Success! Updated Asset {record_id}. I pasted the refreshed card below."
                    if refreshed:
                        agent_output["chat_card"] = cls._build_chat_card(db, "asset", refreshed, mode="edit", language_preference=language_preference)
                else:
                ...
                if res:
                    ...
                    agent_output["text"] = f"Success! Updated Template {record_id}. I pasted the refreshed card below."
                    if refreshed:
                        agent_output["chat_card"] = cls._build_chat_card(db, "message_template", refreshed, mode="edit", language_preference=language_preference)
                else:
