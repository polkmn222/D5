...
function getAiAgentUiCopy() {
    if (aiAgentLanguagePreference === 'kor') {
        return {
            ...
            selectFirstGuide: "먼저 레코드를 선택한 뒤 작업을 선택해 주세요.",
            multiSelectGuidance: "한 번에 하나의 레코드만 열거나 수정할 수 있습니다. 하나만 선택한 뒤 다시 시도해 주세요.",
    ...
    return {
        ...
        selectFirstGuide: "Select a record first, then choose an action.",
        multiSelectGuidance: "Open/Edit works on one record at a time. Select exactly one record, then try again.",
...
function updateSelectionBar() {
    // Selection bar is hidden in favor of table-level controls
...
async function sendAiMessage(queryOverride = null, pageOverride = 1) {
    ...
    try {
        ...
        // Handle auto-open if the flag is set (e.g., after CREATE/UPDATE)
        if (data.auto_open && data.record_id) {
            const rawObj = data.object_type || 'lead';
            const singularObj = rawObj.replace(/s$/, ''); // leads -> lead
            const url = getAgentObjectRoute(singularObj, data.record_id, 'detail');
            if (url) {
                const label = singularObj.charAt(0).toUpperCase() + singularObj.slice(1);
                console.log(`Auto-opening ${label} ${data.record_id}...`);
                openAgentWorkspace(url, `${label} ${data.record_id}`);
            }
        }

        // Immediately remove deleted rows from all visible tables
        if (data.deleted_ids && data.deleted_ids.length > 0) {
            const obj = data.object_type || 'lead';
            data.deleted_ids.forEach(id => removeAgentTableRow(obj, String(id)));
            clearActiveSelection();
            window._agentPendingDelete = null;
        }
    } catch (error) {
        console.error("AI Agent Error Detail:", error);
        const loadingIndicator = document.getElementById(loadingId);
        if (loadingIndicator) loadingIndicator.remove();
...
function appendChatMessage(role, text, id = null, sql = null, results = null, objectType = null, pagination = null, originalQuery = null, chatCard = null) {
    ...
    // Auto-scroll to the top of results, form, or card if one was just added
    setTimeout(() => {
        const resultsCont = msgDiv.querySelector('.results-container');
        const chatCard = msgDiv.querySelector('.agent-paste-card');
        if (resultsCont) {
        ...
        form.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (response.redirected) {
                    ...
                    // Auto-show saved record snapshot and open details
                    const pathParts = finalUrl.pathname.split('/').filter(p => p);
                    if (pathParts.length >= 2) {
                        const urlObject = pathParts[0]; // e.g. 'leads', 'contacts'
                        const urlRecordId = pathParts[1];
                        ...
                        console.log(`Form saved. Resolved type: ${resolvedType}, ID: ${urlRecordId}`);

                        // Automatically open lead details as requested
                        if (resolvedType === 'lead') {
                            const detailUrl = getAgentObjectRoute('lead', urlRecordId, 'detail');
                            if (detailUrl) {
                                console.log(`Auto-opening detail view for Lead ${urlRecordId}...`);
                                openAgentWorkspace(detailUrl, `Lead ${urlRecordId}`);
                            }
                        }
...
function renderAgentChatCard(card) {
    ...
    const editQuery = recordId ? `Manage ${objectType} ${recordId} edit` : '';
    const deleteQuery = recordId ? `Delete ${objectType} ${recordId}` : '';
    const displayName = card.title || 'Lead';
    const actionButtons = recordId ? `
...
function shouldUseAgentChatPaste(objectType) {
    const supported = ['lead', 'contact', 'opportunity', 'brand', 'model', 'product', 'asset', 'message_template', 'template'];
    return supported.includes(objectType);
}
        ...
        <div class="results-container" data-object-type="${objectType || ''}" ${pagination?.table_key ? `data-table-key="${pagination.table_key}"` : ''} data-table-uid="${tableUid}">
            <div class="table-controls">
                ...
                <div class="table-controls-search-row">
                    <input type="text" class="agent-table-search" placeholder="Search in table (Press Enter for Global Search)..." oninput="filterAgentTable(this)" onkeydown="if(event.key === 'Enter') handleAgentGlobalSearch(this)" aria-label="Search in table">
                </div>
                <div class="table-controls-btn-row">
                    ...
                    <button class="control-btn control-btn-action" onclick="triggerSelectionOpenFromTable(this, '${objectType}')">↗️ Open</button>
                    <button class="control-btn control-btn-action" onclick="triggerSelectionEditFromTable(this, '${objectType}')">✏️ Edit</button>
                    <button class="control-btn control-btn-danger" onclick="triggerSelectionDeleteFromTable(this, '${objectType}')">🗑️ Delete</button>
                </div>
...
function handleAgentGlobalSearch(input) {
    if (!input || !input.value.trim()) return;
    const container = input.closest('.results-container');
    if (!container) return;
    const objectType = container.getAttribute('data-object-type') || 'lead';
    const term = input.value.trim();
    
    // Explicitly ask AI for a global search on the object
    const query = `search ${objectType} for "${term}"`;
    sendAiMessage(query);
}
...
function triggerSelectionOpen() {
    const selection = buildSelectionPayload();
    if (!selection) {
        appendChatMessage('agent', getAiAgentUiCopy().selectFirstGuide);
        return;
    }
    if (selection.ids.length !== 1) {
        appendChatMessage('agent', getAiAgentUiCopy().multiSelectGuidance);
        return;
    }
