...
class AIRecommendationService:
    ...
    def _already_refreshed_today(cls, db: Session, reference_date: datetime) -> bool:
        latest = db.query(Opportunity).filter(Opportunity.deleted_at == None).order_by(Opportunity.updated_at.desc()).first()
        if not latest:
            return False
        latest_updated = make_naive_kst(getattr(latest, "updated_at", None)) if getattr(latest, "updated_at", None) else None
        return bool(getattr(latest, "updated_by", None) == "AI Recommend" and latest_updated and latest_updated.date() == reference_date.date())
    ...
    def refresh_opportunity_temperatures(cls, db: Session) -> None:
        ...
        if cls._already_refreshed_today(db, reference_date):
            return
