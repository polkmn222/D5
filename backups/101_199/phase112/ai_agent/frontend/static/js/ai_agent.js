...
async function setDashboardRecommendationMode(mode, btn = null) {
    const trigger = btn || document.getElementById('ai-recommend-btn');
    try {
        ...
        if (typeof showToast === 'function') showToast(`AI Recommend mode changed to ${payload.mode}.`);
        await startAiRecommend(document.getElementById('ai-recommend-btn') || trigger);
    } catch (error) {
        console.error(error);
