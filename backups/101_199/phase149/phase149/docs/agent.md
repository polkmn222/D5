...
- Lead create, open, and edit flows now prefer a chat-embedded pasted card inside the AI Agent conversation instead of relying on the separate workspace for the primary lead CRU path.
...
- Store backups inside a dedicated grouped range folder such as `.gemini/development/backups/001_100/phaseN/` or `.gemini/development/backups/101_199/phaseN/`; do not leave phase backups as loose files at the backups root.
...
- When code or active documentation changes, store pre-change backups under the matching grouped range folder, for example `.gemini/development/backups/101_199/phase148/`, using phase-specific folders and relative paths.
- For docs-only phases, back up every edited active doc before changing it and note clearly if no runtime code changed.
