...
- Copy EVERY file in the repository (excluding `.git/`, `.venv/`, `node_modules/`, and the `backups/` folder itself) into `.gemini/development/backups/<range>/phaseN/` before starting execution.
...
- Preserve the exact relative structure of the entire repository so that each phase backup is a complete, runnable snapshot.
- This full codebase backup is mandatory for every phase, including docs-only phases.
