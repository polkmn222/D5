...
async def test_lead_create_returns_view_card(db: Session):
    ...
    with patch("ai_agent.backend.conversation_context.ConversationContextStore.clear_pending_create"), \
...
async def test_lead_update_returns_view_card(db: Session):
    ...
    with patch("ai_agent.backend.conversation_context.ConversationContextStore.remember_object"):
