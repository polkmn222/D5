...
def test_send_sms_success(db):
    ...
    with patch("web.message.backend.services.messaging_service.MessageProviderFactory.get_provider", return_value=provider):
...
def test_send_mms_success_with_cloudinary_attachment_url(db):
    unique_id = uuid.uuid4().hex[:8]
    att = Attachment(
        id=f"ATT-{unique_id}",
        name="cloud.jpg",
        file_path=f"https://res.cloudinary.com/demo/image/upload/v1/message_templates/cloud-{unique_id}.jpg",
        provider_key=f"cloudinary:message_templates/cloud-{unique_id}",
    )
    db.add(att)
    db.commit()
    ...
    provider = MagicMock()
    provider.provider_name = "mock"
    provider.send.return_value = {"status": "success", "code": "MOCK_OK", "provider_message_id": "mock-4"}
    with patch("web.message.backend.services.messaging_service.MessageProviderFactory.get_provider", return_value=provider):
        msg = MessagingService.send_message(
            db,
            contact_id=db.test_contact_id,
            content="Hello Cloudinary MMS",
            record_type="MMS",
            attachment_id=att.id,
        )
        ...
        assert msg.status == "Sent"
        sent_payload = provider.send.call_args.args[1]
        assert sent_payload.image_url == att.file_path
        assert sent_payload.attachment_provider_key == f"cloudinary:message_templates/cloud-{unique_id}"
