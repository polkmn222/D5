...
const AGENT_TABLE_SCHEMAS = {
    lead: ['first_name', 'last_name', 'phone', 'email', 'status', 'is_followed'],
...
function applyAiAgentLanguageUi() {
    ...
    setText('[data-ai-selection-clear]', copy.clear);
...
function renderAgentResultsMarkup(results, objectType, pagination = null, originalQuery = null) {
    ...
    results.forEach((row, index) => {
        ...
        html += `<tr data-record-id="${rowId}" onclick="selectAgentRecord('${rowId}', '${objectType}', this)" style="cursor: pointer;">
...
function triggerSelectionDelete() {
    ...
    sendQuickMessage(selection.ids.length === 1 ? `Delete ${selection.object_type} ${selection.ids[0]}` : `Delete selected ${selection.object_type} records`);
}
