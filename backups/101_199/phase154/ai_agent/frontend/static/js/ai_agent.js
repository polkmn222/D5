        ...
        form.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (submitMode === 'save-new') {
                    ...
                    const leadMatch = finalUrl.pathname.match(/^\/leads\/([^/?#]+)/);
                    if (leadMatch) {
                        const followData = await fetchAiAgentResponse(`Manage lead ${leadMatch[1]}`);
                        if (followData?.text) {
                            const combinedText = `${successMsg}\n\n${followData.text}`;
                            appendChatMessage('agent', combinedText, null, followData.sql, followData.results, followData.object_type, followData.pagination, followData.original_query, followData.chat_card);
                        } else {
                            appendChatMessage('agent', successMsg);
                        }
                    } else {
...
function openLeadInlineEdit(recordId, recordTitle = 'Lead') {
    if (!recordId) return;
    appendChatMessage('user', `Edit ${recordTitle}`);
    appendAgentInlineFormMessage(
        `I opened the edit form for <strong>${escapeAgentHtml(recordTitle)}</strong> here in chat. Update the fields you want, then save it.`,
        `/leads/new-modal?id=${encodeURIComponent(recordId)}`,
        `Edit ${recordTitle}`,
    );
}
...
function renderAgentChatCard(card) {
    ...
    const actionMarkup = actions.map(action => {
        const toneClass = action.tone === 'danger' ? 'agent-paste-action-danger' : 'agent-paste-action-primary';
        return `<button class="agent-paste-action ${toneClass}" onclick="handleAgentCardAction('${escapeAgentHtml(action.action)}', '${escapeAgentHtml(card.object_type || 'lead')}', '${escapeAgentHtml(card.record_id || '')}', '${escapeAgentHtml(card.title || 'Lead')}')">${escapeAgentHtml(action.label || action.action || 'Action')}</button>`;
    }).join('');
    ...
    return `
        <div class="agent-paste-card" data-agent-card-type="${escapeAgentHtml(card.type)}">
            ...
            <div class="agent-paste-header">
                <div class="agent-paste-header-main">
                    <strong>${escapeAgentHtml(card.title || 'Lead')}</strong>
                    <span>${escapeAgentHtml(card.subtitle || '')}</span>
                </div>
                ${actionMarkup ? `<div class="agent-paste-actions">${actionMarkup}</div>` : ''}
            </div>
...
function handleAgentCardAction(action, objectType, recordId, recordTitle = 'Lead') {
    if (!action || !objectType || !recordId) return;
    const safeTitle = recordTitle || recordId;
    if (action === 'edit') {
        if (objectType === 'lead') {
            openLeadInlineEdit(recordId, safeTitle);
            return;
        }
        sendAiMessageWithDisplay(`Edit ${safeTitle}`, `Manage ${objectType} ${recordId} edit`);
        return;
    }
    if (action === 'delete') {
        sendAiMessageWithDisplay(`Delete ${safeTitle}`, `Delete ${objectType} ${recordId}`);
    }
}
...
function triggerSelectionEdit() {
    ...
    const label = selection.labels?.[0] || selection.ids[0];
    if (shouldUseAgentChatPaste(selection.object_type)) {
        openLeadInlineEdit(selection.ids[0], label);
        return;
    }
