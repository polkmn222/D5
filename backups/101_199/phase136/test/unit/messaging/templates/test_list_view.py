...
from web.message.backend.routers.message_template_router import list_templates
...
async def test_list_templates_includes_content(db, request_mock):
    ...
    with patch("web.message.backend.services.message_template_service.MessageTemplateService.get_templates") as mock_get, \
         patch("web.message.backend.routers.message_template_router.templates.TemplateResponse") as mock_render:
