...
const AGENT_TABLE_SCHEMAS = {
    lead: ['first_name', 'last_name', 'phone', 'email', 'status'],
    contact: ['first_name', 'last_name', 'phone', 'email', 'tier', 'last_interaction_at'],
    opportunity: ['name', 'stage', 'temperature', 'amount', 'status', 'is_followed', 'close_date', 'updated_at'],
    product: ['name', 'brand', 'model', 'category', 'base_price'],
    asset: ['vin', 'name', 'brand', 'model', 'status', 'purchase_date'],
...
function openAgentWorkspace(url, title) {
    ...
    fetch(url)
        ...
        .then(html => {
            ...
            const extracted = doc.querySelector('.sf-modal-header, .detail-header')
                ? (() => {
                    const wrapper = document.createElement('div');
                    const detailHeader = doc.querySelector('.detail-header');
                    const detailTabs = doc.querySelector('.detail-tabs');
                    const detailsTab = doc.querySelector('#tab-details');
                    const relatedTab = doc.querySelector('#tab-related');
                    const modalHeader = doc.querySelector('.sf-modal-header');
                    const modalBody = doc.querySelector('.sf-modal-body');
                    const modalFooter = doc.querySelector('.sf-modal-footer');
                    [detailHeader, detailTabs, detailsTab, relatedTab, modalHeader, modalBody, modalFooter].forEach(node => {
...
function startTemplateSendFromAgent(templateId) {
    ...
    window.location.href = '/messaging/ui?sourceObject=message_template';
}
...
function renderAgentResultsMarkup(results, objectType, pagination = null, originalQuery = null) {
    ...
    const keys = schemaKeys.filter(k => !['id', 'created_at', 'updated_at', 'deleted_at', 'record_id'].includes(k));
    keys.forEach((k, idx) => {
    ...
    results.forEach((row, index) => {
        ...
        const rowLabel = [row.first_name, row.last_name].filter(Boolean).join(' ').trim() || row.name || row.subject || row.phone || row.email || rowId;
...
function getAgentObjectRoute(objectType, recordId, action = 'detail') {
    const routes = {
        lead: { detail: null, edit: `/leads/new-modal?id=${recordId}` },
        contact: { detail: null, edit: `/contacts/new-modal?id=${recordId}` },
        opportunity: { detail: null, edit: `/opportunities/new-modal?id=${recordId}` },
        product: { detail: null, edit: `/products/new-modal?id=${recordId}` },
        asset: { detail: null, edit: `/assets/new-modal?id=${recordId}` },
        model: { detail: null, edit: `/models/new-modal?id=${recordId}` },
