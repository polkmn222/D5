...
def test_dashboard_template_contains_manual_recommend_mode_buttons():
    ...
    assert 'id="dashboard-recommend-mode-toggle"' in html
    assert "Mode: {{ recommendation_mode }}" in html
    assert "cycleDashboardRecommendationMode(this)" in html
    assert "Start Recommend" in html
    assert "Tap to switch mode" in html
