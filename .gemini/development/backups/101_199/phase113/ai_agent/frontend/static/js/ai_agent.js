...
const AI_AGENT_DEFAULT_BODY_HTML = `
    <div id="ai-agent-selection-bar" class="selection-bar is-hidden">
        <div class="selection-bar-copy">
            ...
            <span id="ai-agent-selection-detail" class="selection-bar-detail">Keep chatting to manage these records, or use Edit, Delete, or Send Message when you're ready.</span>
        </div>
        <div class="selection-bar-actions">
            <button class="selection-action-btn" onclick="clearActiveSelection()">Clear</button>
...
function summarizeSelectionIds(ids) {
    if (!ids || !ids.length) return "Keep chatting to manage these records, or use Edit, Delete, or Send Message when you're ready.";
...
function updateSelectionBar() {
    ...
    if (!selection) {
        ...
        detail.textContent = "Keep chatting to manage these records, or use Edit, Delete, or Send Message when you're ready.";
        return;
    }
...
function startTemplateSendFromAgent(templateId) {
    ...
    window.location.href = '/send?sourceObject=message_template';
}
...
function triggerSelectionEdit() {
    ...
    sendQuickMessage(`Manage ${selection.object_type} ${selection.ids[0]}`);
}

function triggerSelectionDelete() {
    const selection = buildSelectionPayload();
    if (!selection) {
        appendChatMessage('agent', 'Select exactly one record to delete it.');
        return;
    }
    if (selection.ids.length !== 1) {
        appendChatMessage('agent', 'Delete works on one record at a time. Select a single record, then try again.');
        return;
    }
    sendQuickMessage(`Delete ${selection.object_type} ${selection.ids[0]}`);
}
...
function updateDashboardRecommendationModeButtons(mode) {
    const btn = document.getElementById('dashboard-recommend-mode-toggle');
    if (!btn) return;
    btn.setAttribute('data-ai-recommend-mode', mode);
...
async function setDashboardRecommendationMode(mode) {
    try {
        ...
        if (typeof showToast === 'function') showToast(`AI Recommend mode changed to ${payload.mode}. Press Start Recommend to refresh.`);
    } catch (error) {
        console.error(error);
...
function cycleDashboardRecommendationMode(btn) {
    const modes = ['Hot Deals', 'High Value', 'Closing Soon', 'New Records'];
    const currentMode = (btn && btn.getAttribute('data-ai-recommend-mode')) || 'Hot Deals';
    const currentIndex = modes.indexOf(currentMode);
    const nextMode = modes[(currentIndex + 1) % modes.length];
    setDashboardRecommendationMode(nextMode);
}
