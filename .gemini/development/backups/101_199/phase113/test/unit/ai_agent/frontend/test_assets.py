...
def test_script_contains_reset_and_pagination_hooks():
    ...
    assert "cycleDashboardRecommendationMode" in js
    assert "Press Start Recommend to refresh" in js
    assert "Delete works on one record at a time" in js
    assert "Manage ${selection.object_type} ${selection.ids[0]}" in js
