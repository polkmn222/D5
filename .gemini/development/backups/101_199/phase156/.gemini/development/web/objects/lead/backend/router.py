...
from web.objects.lead.backend.service import LeadService
...
class LeadRouter:
    """
    LeadRouter manages all routes for the Lead object.
    ...
    async def list_lead_views(db: Session = Depends(get_db)):
        try:
            views = LeadListViewService.list_views(db, LeadRouter.LEAD_LIST_COLUMNS)
            ...
            logger.error(f"Error listing lead views: {str(e)}")
    ...
    async def create_lead_view(payload: dict = Body(...), db: Session = Depends(get_db)):
        try:
            view = LeadListViewService.create_view(db, payload, LeadRouter.LEAD_LIST_COLUMNS)
            return JSONResponse(content={"status": "success", "view": view})
        except ValueError as exc:
            return JSONResponse(status_code=400, content={"status": "error", "message": str(exc)})
        except Exception as e:
            logger.error(f"Error creating lead view: {str(e)}")
    ...
    async def update_lead_view(view_id: str, payload: dict = Body(...), db: Session = Depends(get_db)):
        try:
            view = LeadListViewService.update_view(db, view_id, payload, LeadRouter.LEAD_LIST_COLUMNS)
            return JSONResponse(content={"status": "success", "view": view})
        except ValueError as exc:
            return JSONResponse(status_code=400, content={"status": "error", "message": str(exc)})
        except Exception as e:
            logger.error(f"Error updating lead view {view_id}: {str(e)}")
            return JSONResponse(status_code=500, content={"status": "error", "message": "Internal server error"})
    ...
    async def delete_lead_view(view_id: str, db: Session = Depends(get_db)):
        try:
            ...
            logger.error(f"Error deleting lead view {view_id}: {str(e)}")
            return JSONResponse(status_code=500, content={"status": "error", "message": "Internal server error"})
    ...
    async def pin_lead_view(view_id: str, payload: dict = Body(default=None), db: Session = Depends(get_db)):
        try:
            ...
            logger.error(f"Error pinning lead view {view_id}: {str(e)}")
            return JSONResponse(status_code=500, content={"status": "error", "message": "Internal server error"})
        ...
        try:
            ...
            if prod:
                related_lists.append({
                    ...
                    "view_all_url": f"/products?id={prod.id}&related=1&back_to=/leads/{lead_id}?tab=related",
                    "total_count": 1,
                    "items": [{
                        "name": prod.name or "",
            ...
            if model:
                related_lists.append({
                    ...
                    "view_all_url": f"/models?id={model.id}&related=1&back_to=/leads/{lead_id}?tab=related",
                    "total_count": 1,
                    "items": [{
                        "name": model.name or "",
            ...
            if brand:
                related_lists.append({
                    ...
                    "icon": "brand",
                    "columns": ["Name", "Type"],
                    "view_all_url": f"/vehicle_specifications?id={brand.id}&related=1&back_to=/leads/{lead_id}?tab=related",
                    "total_count": 1,
                    "items": [{
                        "name": brand.name or "",
            ...
            converted_contact = ContactService.get_contact(db, lead.converted_contact) if lead.converted_contact else None
            if converted_contact:
                contact_name = f"{converted_contact.first_name if converted_contact.first_name else ''} {converted_contact.last_name if converted_contact.last_name else ''}".strip() or converted_contact.name or ""
                related_lists.append({
                    "title": "Converted Contact",
                    "icon": "contact",
                    "columns": ["Name", "Email", "Phone"],
                    "view_all_url": f"/contacts?id={converted_contact.id}&related=1&back_to=/leads/{lead_id}?tab=related",
                    "total_count": 1,
                    "items": [{
                        "name": contact_name,
                        "email": converted_contact.email or "",
                        "phone": converted_contact.phone or "",
                        "_href": f"/contacts/{converted_contact.id}",
                    }],
                })

            converted_opportunity = OpportunityService.get_opportunity(db, lead.converted_opportunity) if lead.converted_opportunity else None
            if converted_opportunity:
                related_lists.append({
                    "title": "Converted Opportunity",
                    "icon": "opportunity",
                    "columns": ["Name", "Stage", "Amount"],
                    "view_all_url": f"/opportunities?id={converted_opportunity.id}&related=1&back_to=/leads/{lead_id}?tab=related",
                    "total_count": 1,
                    "items": [{
                        "name": converted_opportunity.name or "",
                        "stage": converted_opportunity.stage or "",
                        "amount": f"{converted_opportunity.amount:,}" if converted_opportunity.amount else "0",
                        "_href": f"/opportunities/{converted_opportunity.id}",
                    }],
                })

            return templates.TemplateResponse(request, "lead/detail_view.html", {
                "request": request,
                "object_type": "Lead",
                "plural_type": "leads",
                "record_id": lead_id,
            ...
            logger.error(f"Error rendering lead detail {lead_id}: {str(e)}")
            return RedirectResponse(url="/leads?error=Internal+server+error")
        ...
        try:
            ...
            if related_mode:
                ...
                return templates.TemplateResponse(request, "related/list_view.html", {
                    ...
                    "page_title": "Leads",
                    "items": items,
                    "columns": LeadRouter.LEAD_LIST_COLUMNS,
                    "back_url": back_url,
                    "new_url": new_url,
                    "page_icon": "lead",
                    "show_actions": True,
            ...
            saved_views = LeadListViewService.list_views(db, LeadRouter.LEAD_LIST_COLUMNS)
            ...
            return templates.TemplateResponse(request, "lead/list_view.html", {
                "request": request,
                "object_type": "Lead",
                "plural_type": "leads",
                "items": items,
    ...
    async def new_lead_form(request: Request):
        return templates.TemplateResponse(request, "lead/create_edit_modal.html", {
            "request": request,
            "object_type": "Lead",
            "is_new": True
        })
    ...
    async def create_lead(
        ...
        try:
            ...
            logger.error(f"Error creating lead via POST: {str(e)}")
            return RedirectResponse(url="/leads?error=Failed+to+create+record", status_code=303)
    ...
    async def update_lead_direct(
        ...
        return await LeadRouter.update_lead(lead_id, first_name, last_name, gender, email, phone, status, brand, model, product, description, db)
    ...
    async def update_lead(
        ...
        try:
            ...
            return RedirectResponse(url=f"/leads/{lead_id}?error=Failed+to+update+record", status_code=303)
    ...
    async def batch_save_lead(lead_id: str, updates: dict, db: Session = Depends(get_db)):
        try:
            ...
            logger.error(f"Error batch saving lead {lead_id}: {str(e)}")
    ...
    async def delete_lead(request: Request, lead_id: str, db: Session = Depends(get_db)):
        try:
            ...
            return RedirectResponse(url="/leads?error=Failed+to+delete+record", status_code=303)
    ...
    async def convert_lead_modal(request: Request, lead_id: str, db: Session = Depends(get_db)):
        try:
            ...
            if not lead:
                return RedirectResponse(url="/leads?error=Lead+not+found")
            return templates.TemplateResponse(request, "lead/lead_convert_modal.html", {
                "request": request,
                "lead": lead
            })
        except Exception as e:
            logger.error(f"Error showing lead convert modal {lead_id}: {str(e)}")
            return RedirectResponse(url=f"/leads/{lead_id}?error=Internal+server+error")
            ...
            if result and "contact" in result:
                if result.get("opportunity"):
                    return RedirectResponse(
                        url=f"/opportunities/{result['opportunity'].id}?success=Lead+converted+successfully",
                        status_code=303,
                    )
                return RedirectResponse(
                    url=f"/contacts/{result['contact'].id}?success=Lead+converted+successfully",
                    status_code=303,
                )
    ...
    async def toggle_lead_follow_endpoint(lead_id: str, enabled: bool = Form(...), db: Session = Depends(get_db)):
        try:
            ...
            logger.error(f"Error toggling lead follow: {e}")
            return {"status": "error", "message": str(e)}
    ...
    async def restore_lead_endpoint(lead_id: str, db: Session = Depends(get_db)):
        try:
            ...
            logger.error(f"Error restoring lead {lead_id}: {e}")
            return {"status": "error", "message": str(e)}
