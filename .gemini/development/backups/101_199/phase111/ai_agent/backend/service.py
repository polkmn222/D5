        ...
        if intent == "MODIFY_UI":
            ...
            if "ai recommend" in q_low or "추천" in q_low:
                if "hot" in q_low or "따끈" in q_low:
                    ...
                    agent_output["text"] = "I've restored the AI Recommendation logic to **Default** (Most recently created sendable deals). Please refresh the dashboard or click [AI Recommend] to see the results!"
                else:
                    ...
                    options = [
                        ...
                        f"[High Value{' (Current)' if current_mode == 'High Value' else ''}]",
                        f"[Closing Soon{' (Current)' if current_mode == 'Closing Soon' else ''}]",
                        f"[Default - Most Recent{' (Current)' if current_mode == 'Default' else ''}]",
                    ]
            ...
            if any(word in q_low for word in ["compact", "축소", "작게"]):
                ...
                agent_output["text"] = f"The current **AI Recommend** logic is **{current_mode}**. How would you like to change it? {' '.join(options)}."
