...
async def get_ai_recommendations(request: Request, db: Session = Depends(get_db)):
    try:
        ...
        recommended_opps = AIRecommendationService.get_sendable_recommendations(db, limit=5)
...
async def set_ai_recommendation_mode(request: Request):
    try:
        ...
        return {"status": "success", "mode": AIRecommendationService.get_recommendation_mode()}
    except Exception as e:
