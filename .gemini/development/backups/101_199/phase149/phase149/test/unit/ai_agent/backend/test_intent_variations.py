...
@pytest.mark.parametrize("query, expected", [
    ...
    ("edit lead", "update a lead"),
    ("delete lead", "delete a lead"),
])
def test_rule_based_crud_guidance_prompts_for_next_step(client, monkeypatch, query, expected):
    async def fail_llm(*args, **kwargs):
        raise AssertionError("LLM fallback should not run for simple CRUD guidance prompts")
    ...
    assert data["intent"] == "CHAT"
    assert expected in data["text"].lower()
