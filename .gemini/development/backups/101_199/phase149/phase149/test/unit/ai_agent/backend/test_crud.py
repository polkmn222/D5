...
async def test_ai_agent_create_lead_conversational(db):
    ...
    with patch.object(AiAgentService, '_call_multi_llm_ensemble', new_callable=AsyncMock) as mock_ensemble:
        ...
        assert "create a lead" in res["text"].lower()
        assert "last name" in res["text"].lower()
...
@pytest.mark.asyncio
async def test_ai_agent_lead_create_followup_guides_and_completes_without_llm(db):
    conversation_id = f"conv-{uuid.uuid4().hex[:8]}"
    ConversationContextStore.clear(conversation_id)

    first = await AiAgentService.process_query(db, "create lead", conversation_id=conversation_id)
    assert first["intent"] == "CHAT"
    assert "create a lead" in first["text"].lower()

    second = await AiAgentService.process_query(db, "a", conversation_id=conversation_id)
    assert second["intent"] == "CHAT"
    assert "still need" in second["text"].lower() or "need" in second["text"].lower()
    assert "status" in second["text"].lower()

    third = await AiAgentService.process_query(db, "last name Kim, status New, email kim@test.com", conversation_id=conversation_id)
    assert third["intent"] == "CREATE"
    assert "Success" in third["text"]

    lead = db.query(Lead).filter_by(last_name="Kim", email="kim@test.com").first()
    assert lead is not None
...
async def test_ai_agent_manage_opportunity(db):
    ...
    with patch.object(AiAgentService, '_call_multi_llm_ensemble', new_callable=AsyncMock) as mock_ensemble:
        ...
        assert any(row["name"] == test_opp_name for row in res["results"])
        assert "text" in res
        assert "opportunit" in res["text"].lower()
