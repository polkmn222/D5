...
from web.message.backend.router import TemplateCreate, create_template
...
async def test_update_template_passes_subject_to_service(db):
    with patch("web.message.backend.router.MessageTemplateService.get_template") as mock_get, \
