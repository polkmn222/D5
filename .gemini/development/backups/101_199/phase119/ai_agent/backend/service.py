...
class AiAgentService:
    ...
    def _default_query_parts(obj: str) -> Optional[Dict[str, str]]:
        mapping = {
            "lead": {
                "select": "id, first_name, last_name, email, phone, status, is_followed, created_at",
                "from": "leads",
    ...
    def _resolve_delete_confirmation(
        ...
        if selected_object and selected_ids:
            ...
            return {
                ...
                "text": (
                    f"Delete confirmation needed: should I permanently delete these {count} {label} records? "
                    "Reply 'yes' to continue or 'cancel' to keep them."
                    if count > 1 else
                    f"Delete confirmation needed: should I permanently delete this {label} record ({selected_ids[0]})? Reply 'yes' to continue or 'cancel' to keep it."
                ),
                "score": 1.0,
            }
