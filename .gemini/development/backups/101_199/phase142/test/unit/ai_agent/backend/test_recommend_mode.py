...
def test_already_refreshed_today_uses_any_same_day_ai_updated_opportunity():
    today = datetime.now()
    older = MagicMock(updated_by="AI Recommend", updated_at=today - timedelta(days=1))
    current = MagicMock(updated_by="AI Recommend", updated_at=today)
    db = MagicMock()
    db.query.return_value.filter.return_value.all.return_value = [older, current]

    assert AIRecommendationService._already_refreshed_today(db, today) is True
