...
class AiAgentService:
    ...
    def _default_query_parts(obj: str) -> Optional[Dict[str, str]]:
        mapping = {
            "lead": {
                "select": "l.id, l.first_name || ' ' || l.last_name AS display_name, l.phone, l.status, m.name AS model, l.created_at",
    ...
    def _apply_search_to_sql(obj: str, config: Dict[str, str], term: str) -> Dict[str, str]:
        ...
        search_fields = {
            "lead": ["l.first_name", "l.last_name", "l.first_name || ' ' || l.last_name", "l.email", "l.phone", "l.status"],
            "contact": ["first_name", "last_name", "first_name || ' ' || last_name", "email", "phone"],
    ...
    def _resolve_delete_confirmation(
        ...
        # Phase 176: If the query is an explicit "Delete {object} {record_id}", 
        # we can assume the user wants to delete it (e.g. from a UI button or explicit command)
        # However, to maintain safety for natural language, we only auto-confirm 
        # if the record_id was explicitly mentioned in the query along with a delete command.
        if record_id and (record_id in user_query or record_id in normalized_query):
             return {
                "intent": "DELETE",
                "object_type": object_type,
                "record_id": record_id,
    ...
    async def process_query(
        ...
        if pending_edit_resolution:
            # Phase 176: If the query is an explicit "edit lead {id}", open the form directly
