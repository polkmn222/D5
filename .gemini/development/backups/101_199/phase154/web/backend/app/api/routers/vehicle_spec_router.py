...
async def model_detail(request: Request, model_id: str, db: Session = Depends(get_db)):
    try:
        ...
        assets = db.query(Asset).filter(Asset.model == model_id, Asset.deleted_at == None).order_by(Asset.name.asc()).all()
        if assets:
            related_lists.append({
                "title": "Assets",
                "icon": "asset",
                "columns": ["Name", "Vin", "Status"],
                "new_url": f"/assets/new-modal?model={model_id}",
                "view_all_url": f"/assets?model={model_id}&related=1&back_to=/models/{model_id}?tab=related",
                "total_count": len(assets),
                "items": [{"name": a.name or "", "vin": a.vin or "", "status": a.status or "", "_href": f"/assets/{a.id}"} for a in assets[:5]],
            })

        leads = db.query(Lead).filter(Lead.model == model_id, Lead.deleted_at == None, Lead.is_converted == False).order_by(Lead.updated_at.desc()).all()
        if leads:
            related_lists.append({
                "title": "Leads",
                "icon": "lead",
                "columns": ["Name", "Email", "Phone"],
                "new_url": f"/leads/new-modal?model={model_id}",
                "view_all_url": f"/leads?model={model_id}&related=1&back_to=/models/{model_id}?tab=related",
        ...
        opportunities = db.query(Opportunity).filter(Opportunity.model == model_id, Opportunity.deleted_at == None).order_by(Opportunity.name.asc()).all()
        if opportunities:
            related_lists.append({
                "title": "Opportunities",
                "icon": "opportunity",
                "columns": ["Name", "Stage", "Amount"],
                "new_url": f"/opportunities/new-modal?model={model_id}",
                "view_all_url": f"/opportunities?model={model_id}&related=1&back_to=/models/{model_id}?tab=related",
                "total_count": len(opportunities),
                "items": [{"name": o.name or "", "stage": o.stage or "", "amount": f"{o.amount:,}" if o.amount else "0", "_href": f"/opportunities/{o.id}"} for o in opportunities[:5]],
            })
