...
SHELL_CHAT = ROOT / "ai_agent" / "frontend" / "static" / "shell" / "js" / "chat.js"
SHELL_WORKSPACE = ROOT / "ai_agent" / "frontend" / "static" / "shell" / "js" / "workspace.js"
LEAD_CARDS = ROOT / "ai_agent" / "frontend" / "static" / "objects" / "lead" / "js" / "cards.js"
LEAD_FORMS = ROOT / "ai_agent" / "frontend" / "static" / "objects" / "lead" / "js" / "forms.js"
LEAD_ACTIONS = ROOT / "ai_agent" / "frontend" / "static" / "objects" / "lead" / "js" / "actions.js"
RECOMMEND_JS = ROOT / "ai_agent" / "frontend" / "static" / "recommend" / "js" / "recommend.js"
MESSAGING_JS = ROOT / "ai_agent" / "frontend" / "static" / "messaging" / "js" / "send_message.js"
...
def test_template_contains_reset_agent_and_quick_guide_cards():
    ...
    assert "ai-agent-workspace-content" in html
...
def test_script_contains_reset_and_pagination_hooks():
    ...
    assert "openLeadInlineEdit" in js
    assert "renderAgentChatCard" in js
    assert "shouldUseAgentChatPaste" in js
    assert "appendAgentInlineFormMessage(" in js
    assert "`/leads/new-modal?id=${encodeURIComponent(recordId)}`" in js
    assert "appendAgentInlineFormMessage" in js
    assert "loadAgentInlineForm" in js
    assert "wireAgentInlineForm" in js
    assert "const combinedText = `${successMsg}\\n\\n${followData.text}`" in js
    assert "fetchAiAgentResponse" in js
    assert "data.intent === 'OPEN_FORM'" in js
    ...
    assert "Choose what to do with ${names[0] || 'this record'}" in js
    assert "/vehicle_specifications/${recordId}" in js
    assert "category', 'base_price'" in js
    assert "/messaging/ui?sourceObject=message_template" in js
    assert "handleAgentCardAction" in js
    assert "agent-paste-actions" in js
...
def test_styles_include_reset_button_and_pagination_classes():
    ...
    assert ".agent-paste-card" in css
    assert ".agent-paste-actions" in css
    assert ".agent-paste-action" in css
    assert ".agent-paste-action-danger" in css
    assert ".agent-paste-body" in css
    assert ".agent-paste-hint" in css
    assert ".agent-inline-form-shell" in css
    assert ".agent-inline-form-loading" in css
    assert ".selection-action-btn:disabled" in css
    assert "flex-wrap: wrap;" in css
    assert ".agent-workspace-content .detail-header" in css


def test_feature_first_frontend_bridge_files_exist():
    expected_files = [
        SHELL_CHAT,
        SHELL_WORKSPACE,
        LEAD_CARDS,
    ...
    for path in expected_files:
        assert path.exists(), f"Missing frontend bridge file: {path}"


def test_feature_first_frontend_bridge_files_reference_runtime_helpers():
    assert "fetchAiAgentResponse" in SHELL_CHAT.read_text()
    assert "appendAgentInlineFormMessage" in SHELL_WORKSPACE.read_text()
    assert "renderAgentChatCard" in LEAD_CARDS.read_text()
    assert "openLeadInlineEdit" in LEAD_FORMS.read_text()
    assert "handleAgentCardAction" in LEAD_ACTIONS.read_text()
    assert "startAiRecommend" in RECOMMEND_JS.read_text()
    assert "triggerSelectionMessaging" in MESSAGING_JS.read_text()
