...
from .record_delete_service import RecordDeleteService
...
class LeadService(BaseService[Lead]):
    ...
    def delete(cls, db: Session, obj_id: str) -> bool:
        try:
            return RecordDeleteService.delete_lead(db, obj_id)
        except Exception as e:
            logger.error(f"Failed to delete lead via base method {obj_id}: {str(e)}")
            raise e
    ...
    def delete_lead(cls, db: Session, lead_id: str) -> bool:
        try:
            return RecordDeleteService.delete_lead(db, lead_id)
        except Exception as e:
