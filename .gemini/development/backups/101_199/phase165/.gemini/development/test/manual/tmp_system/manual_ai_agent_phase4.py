...
# Use a test database
TEST_DB_PATH = Path(__file__).resolve().parents[2] / "databases" / "legacy" / "test_ai_agent_opp.db"
...
async def test_opportunity_crud():
    ...
    try:
        ...
        db.close()
        # Clean up
        if os.path.exists("./test/databases/legacy/test_ai_agent_opp.db"):
            os.remove("./test/databases/legacy/test_ai_agent_opp.db")
