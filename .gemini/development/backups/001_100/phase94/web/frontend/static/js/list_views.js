...
function initializeLeadListView(config) {
    ...
    function renderFilterConditions(conditions) {
        ...
        setupFilterList.innerHTML = normalizedConditions.map((condition) => `
            <div class="sf-list-view-filter-row">
                ...
                <input class="sf-list-view-form-input" data-filter-value value="${condition.value.replace(/"/g, "&quot;")}" placeholder="Filter value">
    ...
    window.toggleLeadListViewPin = function toggleLeadListViewPin() {
        ...
        confirmLeadListAction(
            shouldPin ? "Pin list view" : "Unpin list view",
            shouldPin ? `Pin ${activeView.label} as your default Lead list view?` : `Remove ${activeView.label} as your pinned Lead list view?`,
            async () => {
                try {
                    const data = await requestLeadListView(`/leads/views/${activeView.id}/pin`, "POST", { pinned: shouldPin });
                    applyPinnedState(data.pinned_view_id || "");
                    renderView();
                    if (typeof showToast === "function") {
                        showToast(shouldPin ? `Pinned ${activeView.label}.` : "Pin removed.");
                    }
                } catch (error) {
                    if (typeof showToast === "function") {
