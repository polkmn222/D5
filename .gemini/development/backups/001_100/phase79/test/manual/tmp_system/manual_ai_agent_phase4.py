...
async def test_opportunity_crud():
    ...
    try:
        ...
        response = await AiAgentService.process_query(db, "Show me opportunities with amount greater than 10,000,000")
        ...
        create_response = await AiAgentService.process_query(db, "Create a new opportunity for Big Sale with amount 100,000,000")
        ...
        if big_sale:
            ...
            update_response = await AiAgentService.process_query(db, f"Change Big Sale ({big_sale.id}) stage to Closed Won")
