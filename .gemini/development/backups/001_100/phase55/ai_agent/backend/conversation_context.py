...
class ConversationContextStore:
    ...
    @classmethod
    def remember_pending_delete(cls, conversation_id: Optional[str], object_type: str, record_id: str) -> None:
        if not conversation_id or not object_type or not record_id:
            return
        context = cls._store.setdefault(conversation_id, {})
        context["pending_delete"] = {"object_type": object_type, "record_id": record_id}
    ...
    def remember_selection(cls, conversation_id: Optional[str], selection: Optional[Dict[str, Any]]) -> None:
        ...
        context = cls._store.setdefault(conversation_id, {})
        context["selection"] = {"object_type": object_type, "ids": list(ids)}
