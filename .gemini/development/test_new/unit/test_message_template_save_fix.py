...
from backend.app.api.routers.message_template_router import update_template_route
from backend.app.api.routers.utility_router import batch_save
...
async def test_update_template_route_content(db):
    ...
    with patch("backend.app.services.message_template_service.MessageTemplateService.update_template") as mock_update:
        # 1. Test with 'content'
...
async def test_batch_save_template_content(db, request_mock):
    ...
    with patch("backend.app.services.message_template_service.MessageTemplateService.get_template") as mock_get, \
         patch("backend.app.services.message_template_service.MessageTemplateService.update_template") as mock_update:
